...
class MCMCDiagnostics:
    ...
    def __init__(self, sampler: MetropolisHastings):
    ...
    def _compute_autocorr(self, param_idx: int, max_lag: Optional[int] = None) -> np.ndarray:
    ...
    def _calculate_diagnostics(self) -> Dict:
        ...
        # Advanced statistics
        ess = np.array([self._compute_ess(self.chain[:, i]) 
                       for i in range(self.n_params)])
        act = np.array([self._compute_act(self.chain[:, i]) 
                       for i in range(self.n_params)])
    ...
    def _plot_traces(self, ax: plt.Axes):
        ...
        for i in range(self.n_params):
            ...
            running_mean = np.cumsum(self.chain[:, i]) / np.arange(1, self.n_samples + 1)
...
#        ax.plot(range(0, self.n_samples-window, window), acc_rates)
        ax.plot(range(0, self.n_samples), self.sampler.acceptance_rates[: self.n_samples])
    ...
    def _plot_ess(self, ax: plt.Axes):
        ...
        ax.bar([f'X{i+1}' for i in range(self.n_params)], 
    ...
    def _plot_marginals(self, ax: plt.Axes):
        ...
        for i in range(self.n_params):
            ax.hist(self.chain[:, i], bins=50, density=True, 
    ...
    def plot_target_distribution(self):
        ...
        plt.scatter(x_samples, y_samples, alpha=0.1, color='k', s=1, label='MCMC samples')
