...
class MALAProposal(Proposal):
    ...
    def __init__(self, proposal_distribution: sp.stats.rv_continuous, scale: float, gradient_func: Callable):
        """
    ...
    def proposal_log_density(self, proposed: np.ndarray, current: np.ndarray) -> np.float64:
        ...
        return np.sum(sp.stats.norm.logpdf(proposed, loc=mean, scale=np.sqrt(self.epsilon)))
...
class AdaptiveMALA(MALA):
    ...
    def update_preconditioner(self, new_sample: np.ndarray):
        ...
        cov_update = (n / (n + 1)) * self.preconditioner + \
            (n / ((n + 1) ** 2)) * np.outer(old_mean - new_sample, old_mean - new_sample)
...
class GaussianTarget(TargetDistribution):
    def __init__(self):
        prior = sp.stats.norm(0, 1)
        likelihood = sp.stats.norm(0, 1)
    ...
    def log_prior(self, x: np.ndarray) -> float:
        """2D standard normal prior"""
        return float(np.sum([self.prior.logpdf(xi) for xi in x]))

    def log_likelihood(self, x: np.ndarray) -> float:
        """2D Gaussian likelihood"""
        return float(np.sum([self.likelihood.logpdf(xi) for xi in x]))
