...
#np.full(nx, 0.1)#sp.stats.uniform(loc=0, scale=0.001).rvs(size=nx)
    ...
    class HeatEquationPrior2(TargetDistribution):
        def __init__(self, x_grid, observations, noise_level=0.03, variance = 0.12):
            """
            ...
            self.true_initial = lambda x: np.sin(3*np.pi * x)  # True initial condition
    ...
    target = HeatEquationPrior2(x_grid, obs)
...
if __name__ == "":
    ...
    class HeatEquationPrior(TargetDistribution):
        def __init__(self, x_grid, observations, noise_level=0.07, variance = 0.05):
            """
            Create a target for the heat equation initial condition
            """
            # Store grid information
            self.x_grid = x_grid
            self.L = x_grid[-1] - x_grid[0]  # Domain length
            self.n_terms = 40  # Number of terms in Fourier series
...
if __name__ == "":
    ...
    # Setup grid and create target distribution
    L = 1.0  # Domain length
    nx = 300  # Number of grid points (also number of datapoints right now which needs to change)
    x_grid = np.linspace(0, L, nx)
    t_grid = sp.stats.gamma.rvs(0.5, scale=1, loc=0, size=nx)/10
#np.full(nx, 0.1)#sp.stats.uniform(loc=0, scale=0.001).rvs(size=nx)
    print(t_grid)
    obs = (x_grid, t_grid)
